...
export const printBill = (item: any) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      await BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.CENTER)
      await BluetoothEscposPrinter.printText(`${item.merchant.name}\n\r`, options)
      await BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.CENTER)
      await BluetoothEscposPrinter.printText(`${item.merchant.address}\n\r`, options)
      await BluetoothEscposPrinter.printText(`${item.merchant.phone_number}\n\r`, options)
        ...
        (p: any, x: any) =>
          p.then(() => {
            return new Promise(async resolve => {
              await BluetoothEscposPrinter.printText(x.product.name.substr(0, 31) + '\n', options)
              await BluetoothEscposPrinter.printColumn(
                columnWidths,
                [BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.RIGHT],
                [`${x.qty}x Rp ${convertToRupiah(x.sub_total / x.qty)}`, `Rp ${convertToRupiah(x.sub_total)}`],
                options,
              )
      ...
      await BluetoothEscposPrinter.printText(dashSeparator, {})
      const calculate = calculateCart(item.transaction_product.map((e: any) => ({...e, ...e.product})))
      if (item.status !== 'hold') {
      ...
      if (item.merchant?.footer_note) {
        await BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.CENTER)
        await BluetoothEscposPrinter.printText(`${item.merchant?.footer_note}\n\r`, options)
      }
      if (item.status === 'hold') {
        await BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.CENTER)
        await BluetoothEscposPrinter.printText('Struk Tagihan - Belum Lunas\n\r', options)
      }
